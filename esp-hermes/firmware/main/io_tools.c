...
#include "esp_adc/esp_adc_cali.h"
#include "esp_adc/esp_adc.h"
#include "esp_log.h"
...
eh_tool_result_t io_tool_exec(eh_tool_t tool, const cJSON *params) {
    ...
    case EH_TOOL_ADC_READ: {
        ...
        if (ch < 0) { r.error = "pin not on ADC1"; break; }
        adc_oneshot_unit_handle_t unit;
        adc_oneshot_unit_init_cfg_t u = { .unit_id = ADC_UNIT_1 };
        if (adc_oneshot_new_unit(&u, &unit) != ESP_OK) { r.error = "adc init"; break; }
        adc_oneshot_chan_cfg_t c = { .atten = ADC_ATTEN_DB_11,
                                      .bitwidth = ADC_BITWIDTH_12 };
        if (adc_oneshot_config_channel(unit, ch, &c) != ESP_OK) {
            r.error = "adc cfg"; adc_oneshot_del_unit(unit); break;
        }
        int raw = 0;
        if (adc_oneshot_read(unit, ch, &raw) == ESP_OK) {
            r.value_i = raw; r.ok = true;
        } else r.error = "adc read";
    ...
    case EH_TOOL_IMU_READ: {
        ...
        r.value_s = malloc(64);
        sprintf(r.value_s, "%.2f,%.2f,%.2f", ax, ay, az);
        r.ok = true;
        break;
    }
    default:
