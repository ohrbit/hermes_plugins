 ...
 * Stick S3: SCL=G48, SDA=G47, INT=G4 (shared bus with ES8311 + M5PM1).
 */
...
/* BMI270 register map (subset) */
#define BMI270_CHIP_ID   0x24
#define REG_CHIP_ID     0x00
#define REG_ACC_DATA    0x0C   /* X LSB, 16-bit LE each axis */
...
void imu_motion_init(void) {
    ...
    i2c_reg_write(REG_PWR_CTRL, 0x0E);   /* enable acc+gyr */
    i2c_reg_write(REG_CMD, 0x02);        /* soft reset */
}

bool imu_read_accel(float *ax, float *ay, float *az) {
    /* BMI270 accel range +/-8g → 4096 LSB/g (after conf) */
...
bool imu_read_gyro(float *gx, float *gy, float *gz) {
    *gx = read_axis(REG_GYR_DATA) / 16.4f;   /* +/-2000dps */
...
void imu_motion_task(void *pvParameters) {
    ...
    for (;;) {
        ...
        /* flip = z-axis inverted (device upside down) */
